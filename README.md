...
00:30:38               updater.exe → Chrome installer unpacking
...
Each event carries a timestamp, event type, ATT&CK technique tag, confidence score, and PIVOT marker. The right panel shows the full ATT&CK technique coverage matrix for the incident with event counts and pivot counts per technique.

---

### Raw Alerts Feed
![Alerts](docs/screenshots/chronos-raw-alerts-mitre-classification.png)
200 most recent alerts with agent name, rule ID, severity level badge, description, and MITRE technique tag. Multiple agents visible — Windows11-Host and DESKTOP-5D0PC8C.

---

### IOC Registry
![IOCs](docs/screenshots/chronos-ioc-extraction-indicators.png)
15 deduplicated indicators with type badges, incident links, hit counts, first/last seen timestamps, and notes. SHA256 and MD5 hashes highlighted for `SecEdit.exe` and `Taskmgr.exe` — both flagged for suspicious execution context.
...
**The problem:**
After configuring the Wazuh agent on the Windows 10 VM and verifying Sysmon events in Event Viewer, the next step was to confirm alerts were reaching the manager. Running `sudo tail -f /var/ossec/logs/alerts/alerts.json` on WSL2 Ubuntu returned `No such file or directory`. Running `sudo systemctl status wazuh-manager` on the Arch Linux VM returned `Unit wazuh-manager.service could not be found`. The manager appeared to not exist anywhere.

**Root cause:**
The Wazuh stack was deployed as a Docker Compose single-node setup, not as a native systemd service. The alerts file existed inside the container, not on the host filesystem directly. Additionally, the Arch Linux VM had been powered off, making the agent's configured server IP unreachable and causing the agent to appear disconnected.

**Fix:**
Ran `docker ps` to identify the running containers (`wazuh/wazuh-manager:4.7.5`, `wazuh/wazuh-indexer`, `wazuh/wazuh-dashboard`). Powered on the Arch VM, confirmed its IP matched the agent config, then accessed the alerts log inside the container with `docker exec -it single-node-wazuh.manager-1 tail -f /var/ossec/logs/alerts/alerts.json`. Sysmon events appeared immediately.
...
**The problem:**
`sudo find /var/lib/docker/volumes -name "alerts.json"` found the file at `/var/lib/docker/volumes/single-node_wazuh_logs/_data/alerts/alerts.json`. The path was updated in `ingest.py`. But running `python ingest.py` still returned `Alerts file not found`. Running `ls -la` on the path returned `Permission denied`.

**Root cause:**
The Docker volume directory tree was owned by root with `drwxr-x---` permissions — the directory existed and the file existed, but the user account running the ingester had no execute (traversal) permission on the parent directories in the chain. `Path.exists()` returned `False` not because the file was missing but because the user couldn't traverse the path to reach it.
...
```
Verified with `head -1` on the file as the normal user before running the ingester.
...
**The problem:**
Running `pip install psycopg2-binary` returned `error: externally-managed-environment` and refused to install. Arch Linux enforces PEP 668, which prevents pip from installing packages system-wide to protect the system Python installation.

**Fix:**
Created a virtual environment scoped to the Chronos project:
```bash
cd ~/chronos
python -m venv venv
...
```
Added `source ~/chronos/venv/bin/activate` as a required step in the project setup. For production systemd service deployment, the unit file will reference `~/chronos/venv/bin/python` directly so activation is automatic.

**Lesson:** On Arch Linux (and increasingly on other distros), never install pip packages system-wide. Always use a venv. The error message itself tells you exactly what to do.
...
**The problem:**
The ingester ran successfully and inserted 832 alerts on first run, but logged repeated warnings: `Normalized insert failed (EventID 13): insert_registry_event() missing 1 required positional argument: 'eid'`. Registry events were being routed to the handler but the event ID wasn't being passed through, so none of them normalized into `registry_events`.

**Root cause:**
The `insert_registry_event` function requires `eid` to map EventID 12/13/14 to human-readable labels (`CreateKey`, `SetValue`, `DeleteKey`). The generic handler dispatch in `process_alert` called all handlers with the same four arguments and didn't pass `eid`. The fix attempt overcorrected — passing `eid` to all handlers — which then broke the process/network/file handlers that don't accept a fifth argument.

**Fix:**
Added conditional dispatch: pass `eid` only when routing to registry handlers (EventID 12, 13, 14), use the standard four-argument call for all others:
```python
if eid in (12, 13, 14):
...
**Lesson:** When fixing a missing argument error, don't add the argument globally — trace which handlers actually need it and scope the fix precisely.
...
**The problem:**
After applying the registry handler fix in nano, running `python ingest.py` returned `IndentationError: unexpected indent` at line 264. The file had been edited with inconsistent spacing — the `if eid in SYSMON_HANDLERS:` line had five leading spaces instead of four, breaking Python's indentation rules.

**Root cause:**
Nano does not enforce consistent indentation. When patching a specific line in a large file via a terminal editor, it is easy to introduce a single extra space that Python treats as a block-level error.

**Fix:**
Used `grep -n` to locate the exact line, then `sed -i` to replace the five-space indent with four spaces:
```bash
sed -i 's/^     if eid in SYSMON_HANDLERS:/    if eid in SYSMON_HANDLERS:/' ~/chronos/ingester/ingest.py
```
Verified with `sed -n '262,266p'` before rerunning.

**Lesson:** For single-line patches in terminal editors, use `sed -i` for precision. Always verify the surrounding lines after any manual edit.
...
**The problem:**
Running `python app.py` returned `SyntaxError: invalid decimal literal` at line 191 pointing to `font-size: 14px;`. The CSS from the HTML template had been accidentally written into the Python file during a shell session where multiple file writes were interleaved.

**Root cause:**
Shell heredocs (`<< 'EOF'`) are unreliable in zsh for large multi-file content. When pasting large blocks of content across multiple nano sessions in rapid succession, content from one file bled into another. Nothing in the error message pointed to CSS as the cause — it took direct inspection of line 191 to identify it.

**Fix:**
Confirmed the corruption with `python -c "import ast; ast.parse(open('app.py').read())"`. Overwrote the file completely using a Python writer script with a raw triple-quoted string, which is immune to shell interpretation:
```bash
python3 - << 'PYEOF'
content = '''... entire Flask app ...'''
with open('/home/cpt-ferna02/chronos/dashboard/app.py', 'w') as f:
    f.write(content)
PYEOF
```
Validated with the AST parser before running.

**Lesson:** Always validate Python files with `ast.parse` before running them. Use Python writer scripts for large files — they are immune to shell escaping and heredoc failures. Never assume a file was written correctly without checking it.
...
**The problem:**
After initializing the local repo and committing all Chronos files, `git push -u origin main` was rejected with `Updates were rejected because the remote contains work that you do not have locally`. The GitHub repo had been initialized with a README on the web interface, creating a commit history that diverged from the local repo.

**Root cause:**
Two independent git histories: one created locally with `git init`, one created by GitHub when the repo was initialized with a README. Neither was aware of the other, so git refused to push without reconciliation.

**Fix:**
```bash
git pull origin main --allow-unrelated-histories --no-rebase
git checkout --ours README.md   # keep local README, discard GitHub's placeholder
git add README.md
git commit -m "Merge: keep local README"
git push -u origin main
```

**Lesson:** When pushing a local project to a GitHub repo that was initialized with any files (README, license, .gitignore), always pull with `--allow-unrelated-histories` first. Initialize GitHub repos empty when you already have local content.
...
- **Database Engineering** — PostgreSQL schema design, multi-table normalized storage, GIN indexes for JSONB and array fields, full-text search on command lines
- **Python Development** — file tailing with cursor tracking, JSON normalization, Flask routing, dataclass-based data modeling, ACL-aware file access
- **Full-Stack Security Tooling** — end-to-end platform from raw telemetry ingestion to investigation UI
- **Systematic Debugging** — diagnosing permission errors, indentation bugs, heredoc failures, git history conflicts, and argument dispatch errors across a multi-component stack
